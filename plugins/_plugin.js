...
// Import Giphy API helper
const { getGiphyGif } = require("./Utilis/giphy");

Asena.addCommand(
  { pattern: "plugin ?(.*)", fromMe: true, desc: Lang.INSTALL_DESC },
  async (message, match) => {
    match = match || message.reply_message.text;
    if (!match && match !== "list") return await message.sendMessage(Lang.NEED_URL);
    if (match == "list") {
      const plugins = await getPlugin();
      if (!plugins) return await message.sendMessage(Lang.NO_PLUGIN);
      return await message.sendMessage(`${Lang.INSTALLED_FROM_REMOTE}\n${plugins}`);
    }
      ...
      try {
        ...
        if (res.statusCode == 200) {
          ...
          try {
            ...
            await message.sendMessage(Lang.INVALID_PLUGIN + "```\n" + e.stack + "```");
            return fs.unlinkSync("./plugins/" + plugin_name + ".js");
          }
          await installPlugin(url, plugin_name);
          await message.sendMessage(Lang.INSTALLED.format(pluginList(res.body).join(",")));
        }
      } catch (error) {
...
Asena.addCommand(
  ...
  async (message, match) => {
    if (message.mention !== false) {
      ...
      try {
        // Fetch a random anime kiss GIF from Giphy
        const gifUrl = await getGiphyGif("anime kiss");
        
        // Send the GIF with a message
        await message.sendMessage(gifUrl, { caption: `${mentionedUser} received a kiss! (Reio wasta :3)` });
      } catch (error) {
        await message.sendMessage("Failed to fetch a kiss GIF. Please try again later.");
      }
    } else {
      await message.sendMessage("Please mention a user to send a kiss GIF.");
    }
  }
);
